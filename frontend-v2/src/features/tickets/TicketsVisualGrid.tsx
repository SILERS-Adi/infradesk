    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {items.map((t) => (
        <Link key={t.id} to={`/tickets/${t.id}`} className="block group">
          <Card className="p-4 h-full hover:border-[var(--bd-f)] transition-colors">
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="flex items-center gap-2">
                <span className={`h-2.5 w-2.5 rounded-full ${PRIORITY_DOT[t.priority] ?? 'bg-tm'}`} aria-label={`Priorytet ${t.priority}`} />
                <span className="text-xs font-semibold text-tx3">{t.ticketNumber}</span>
              </div>
              {statusBadge(t.status)}
            </div>
            <h3 className="text-sm font-medium text-tx line-clamp-2 mb-3 min-h-[2.5rem] group-hover:text-pri transition-colors">{t.title}</h3>
            <div className="flex items-center gap-2 text-xs text-tx3 flex-wrap">
