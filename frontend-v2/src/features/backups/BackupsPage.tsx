...
export function BackupsPage() {
  ...
  return (
    <div className="space-y-5 anim-up">
      ...
      {showCreate && <BackupWizard open={showCreate} onClose={() => setShowCreate(false)} />}
    </div>
  );
}
...
function _CreateBackupModal({ onClose }: { onClose: () => void }) {
  const qc = useQueryClient();
...
// keep legacy modal exported as void to retain code reference
void _CreateBackupModal;
