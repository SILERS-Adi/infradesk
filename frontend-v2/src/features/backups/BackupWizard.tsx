...
const inputStyle: React.CSSProperties = {
  ...
  background: 'var(--sf-h)', border: '1px solid var(--bd)', color: 'var(--tx)', outline: 'none',
};
const labelStyle: React.CSSProperties = {
  display: 'block', fontSize: 11, fontWeight: 600, color: 'var(--tx2)',
  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer">
      <div
        ...
        style={{ background: value ? 'linear-gradient(135deg, #4f8cff 0%, #6366F1 40%, #8B5CF6 100%)' : 'var(--bd)' }}
...
function StepProgress({ current, steps }: { current: Step; steps: typeof STEPS }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 0, padding: '16px 24px', borderBottom: '1px solid var(--bd)' }}>
      {steps.map((s, i) => {
        const done = i < idx;
        const active = i === idx;
        return (
          <div key={s.key} style={{ display: 'flex', alignItems: 'center', flex: i < steps.length - 1 ? 1 : 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{
                ...
                background: done ? '#22C55E' : active ? 'var(--pri)' : 'var(--sf-h)',
                color: done || active ? '#fff' : 'var(--tx3)',
                border: active ? 'none' : done ? 'none' : '1px solid var(--bd)',
              }}>
                {done ? <Check style={{ width: 12, height: 12 }} /> : i + 1}
              </div>
              <span style={{
                fontSize: 11, fontWeight: active ? 700 : 500, whiteSpace: 'nowrap',
                color: active ? 'var(--tx)' : done ? '#22C55E' : 'var(--tx3)',
              }}>
                {s.label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div style={{ flex: 1, height: 1, margin: '0 10px', background: done ? '#22C55E' : 'var(--bd)' }} />
            )}
          </div>
        );
      })}
  ...
  return (
    <button
      ...
      style={{
        ...
        border: selected ? `2px solid ${color || 'var(--pri)'}` : '1px solid var(--bd)',
        background: selected ? `${color || 'var(--pri)'}08` : 'transparent',
        opacity: disabled ? 0.4 : 1, transition: 'all 0.15s',
      }}
      onMouseEnter={e => { if (!disabled && !selected) { e.currentTarget.style.borderColor = color || 'var(--pri)'; e.currentTarget.style.background = 'var(--sf-h)'; } }}
      onMouseLeave={e => { if (!disabled && !selected) { e.currentTarget.style.borderColor = 'var(--bd)'; e.currentTarget.style.background = 'transparent'; } }}
    >
      <div style={{
        width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center',
        background: `${color || 'var(--pri)'}15`, flexShrink: 0,
      }}>
        {icon}
      </div>
...
function SumRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--bd)' }}>
      ...
      <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx2)', width: 120, flexShrink: 0 }}>{label}</div>
      <div style={{ fontSize: 13, color: 'var(--tx)', flex: 1 }}>{value}</div>
    </div>
  );
}
...
export default function BackupWizard({ open, onClose, companyFilter }: { open: boolean; onClose: () => void; companyFilter?: string }) {
  ...
  const { data: agents = [] } = useQuery({
    queryKey: ['agents'],
    queryFn: async () => (await api.get('/agents/admin?status=ACTIVE')).data.agents,
    enabled: open,
  });

  const activeAgents = agents.filter((a: any) =>
    a.status === 'ACTIVE'
    && (!companyFilter || a.workspaceId === companyFilter)
    && (!selectedWorkspace || a.workspaceId === selectedWorkspace)
  );

  // Build company list from workspaces present on agents (deduplicated by id).
  const companies: Array<{ id: string; name: string }> = (() => {
    const m = new Map<string, string>();
    for (const a of agents) {
      if (!a.workspaceId) continue;
      if (companyFilter && a.workspaceId !== companyFilter) continue;
      const name = a.workspace?.name || a.companyName || '?';
      if (!m.has(a.workspaceId)) m.set(a.workspaceId, name);
    }
  ...
  const scanMut = useMutation({
    mutationFn: (agentId: string) =>
      api.post(`/agents/admin/${agentId}/push-command`, { type: 'scan_databases', timeoutMs: 60000 }).then(r => r.data.ack),
  ...
  const testMut = useMutation({
    mutationFn: () =>
      api.post(`/agents/admin/${selectedAgent.id}/push-command`, { type: 'test_db_connection', timeoutMs: 30000, payload: {
  ...
  const createMut = useMutation({
    mutationFn: () => {
      ...
      return api.post('/backups', {
        ...
        agentRegistrationId: selectedAgent.id,
  ...
  const handleGdriveAuth = async () => {
    try {
      const { url } = await api.get('/backups/google-auth-url').then(r => r.data);
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(o) => !o && reset()}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm anim-up" />
        <Dialog.Content
          className="fixed left-1/2 top-[2vh] z-50 w-full max-w-3xl -translate-x-1/2 rounded-[var(--r-xl)] anim-scale max-h-[96vh] overflow-hidden flex flex-col"
        ...
        >
          <div className="flex items-center justify-between px-6 py-4 border-b border-bd shrink-0">
            <Dialog.Title className="text-[16px] font-bold text-tx">Kreator kopii zapasowej</Dialog.Title>
            <Dialog.Close asChild>
              <button className="p-2 rounded-[var(--r-s)] text-tx3 hover:bg-sf-h press" aria-label="Zamknij">
                <X className="h-4 w-4" />
              </button>
            </Dialog.Close>
          </div>
      ...
      <div style={{ minHeight: 460 }}>
        ...
        {/* Security badge */}
        <div style={{ padding: '10px 24px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid var(--bd)', background: 'rgba(34,197,94,0.03)' }}>
          <Shield size={13} color="#22C55E" />
          <span style={{ fontSize: 10, color: '#22C55E', fontWeight: 600 }}>Szyfrowane połączenie</span>
          <span style={{ fontSize: 10, color: 'var(--tx3)' }}>— dane logowania szyfrowane AES-256, nie przechowywane w przeglądarce</span>
        </div>
        ...
        <div style={{ padding: 24 }}>
          ...
          {step === 'source' && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Co chcesz zabezpieczyć?</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 20px' }}>Wybierz typ danych do kopii zapasowej</p>
              ...
              {companies.length > 1 && (
                <div style={{ marginBottom: 14 }}>
                  <label style={labelStyle}>Firma klienta</label>
                  <select
                    value={selectedWorkspace}
                    onChange={(e) => { setSelectedWorkspace(e.target.value); setSelectedAgent(null); }}
                    style={inputStyle}
                  >
                    <option value="">Wszystkie firmy ({companies.length})</option>
                    {companies.map((co) => (
              ...
              <h4 style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)', marginBottom: 10 }}>Wybierz maszynę</h4>

              {activeAgents.length === 0 ? (
                <div style={{ padding: 32, textAlign: 'center', borderRadius: 14, border: '1px solid var(--bd)' }}>
                  <WifiOff size={32} color="var(--tx3)" style={{ marginBottom: 8 }} />
                  <p style={{ fontSize: 13, color: 'var(--tx2)' }}>Brak zarejestrowanych asystentów</p>
                  <p style={{ fontSize: 11, color: 'var(--tx3)' }}>Zainstaluj Asystent Business na maszynie klienta</p>
                </div>
              ) : (
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 12, border: '1px solid var(--bd)', background: 'transparent',
                          cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s',
                          opacity: online ? 1 : 0.7,
                        }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--pri)'; e.currentTarget.style.background = 'var(--sf-h)'; }}
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)' }}>{agent.hostname}</div>
                        ...
                        {scanMut.isPending && selectedAgent?.id === agent.id
                          ? <Loader2 size={14} className="animate-spin" style={{ color: 'var(--pri)' }} />
          ...
          {step === 'scan' && sourceType === 'sql' && scanResult && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Wykryte bazy danych</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 20px' }}>
                Maszyna: <strong>{scanResult.hostname}</strong> 
              </p>

              {scanResult.databases.length === 0 && scanResult.services.length === 0 ? (
                <div style={{ padding: 32, textAlign: 'center', borderRadius: 14, border: '1px solid var(--bd)' }}>
                  <AlertTriangle size={32} color="#F59E0B" style={{ marginBottom: 8 }} />
                  <p style={{ fontSize: 13, color: 'var(--tx2)', fontWeight: 600 }}>Nie wykryto żadnych baz danych</p>
                  <p style={{ fontSize: 11, color: 'var(--tx3)', marginTop: 4 }}>Sprawdź czy serwer baz danych jest uruchomiony i nasłuchuje na standardowym porcie</p>
                </div>
              ) : (
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 14, border: '1px solid var(--bd)', background: 'transparent',
                          cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s',
                        }}
              ...
              {scanResult.services.length > 0 && (
                <div style={{ marginTop: 20 }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: 'var(--tx2)', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 8 }}>
                    Usługi baz danych
                  </p>
              ...
              <button onClick={() => setStep('source')} style={{ marginTop: 20, fontSize: 12, color: 'var(--pri)', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {step === 'scan' && sourceType === 'folder' && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Ścieżka folderu</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 20px' }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <p style={{ fontSize: 10, color: 'var(--tx3)', marginTop: 4 }}>
          ...
          {step === 'credentials' && selectedDb && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>
                    ...
                    {([['sql', 'SQL Server', 'Logowanie kontem SQL (sa)'], ['windows', 'Windows', 'Uwierzytelnianie kontem Windows (Trusted)']] as const).map(([mode, title, desc]) => (
                      <button key={mode} onClick={() => {
                        ...
                        style={{
                          ...
                          border: authMode === mode ? '2px solid var(--pri)' : '1px solid var(--bd)',
                          background: authMode === mode ? 'rgba(99,102,241,0.06)' : 'transparent',
                          transition: 'all 0.1s',
                        }}
                        onMouseEnter={e => { if (authMode !== mode) e.currentTarget.style.background = 'var(--sf-h)'; }}
                        onMouseLeave={e => { if (authMode !== mode) e.currentTarget.style.background = 'transparent'; }}
                      >
                        <div style={{ fontSize: 13, fontWeight: 600, color: authMode === mode ? 'var(--pri)' : 'var(--tx)' }}>{title}</div>
          ...
          {step === 'databases' && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Wybierz bazy danych</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 16px' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                <button onClick={() => setSelectedDbs([...dbList])}
                  style={{
                    ...
                    border: '1px solid var(--bd)', background: 'var(--sf-h)', color: 'var(--tx)',
                    transition: 'all 0.1s',
                  }}>
                  Zaznacz wszystkie
                </button>
                ...
                <button onClick={() => setSelectedDbs([])}
                  style={{
                    fontSize: 11, fontWeight: 600, padding: '6px 14px', borderRadius: 8, cursor: 'pointer',
                    border: '1px solid var(--bd)', background: 'var(--sf-h)', color: 'var(--tx3)',
                    transition: 'all 0.1s',
                  }}>
                  Odznacz wszystkie
                </button>
                  ...
                  return (
                    <button key={db} onClick={() => setSelectedDbs(prev => sel ? prev.filter(d => d !== db) : [...prev, db])}
                      style={{
                        ...
                        border: sel ? '1px solid rgba(139,92,246,0.3)' : '1px solid var(--bd)',
                        background: sel ? 'rgba(139,92,246,0.06)' : 'transparent',
                        transition: 'all 0.1s',
                      }}
                      onMouseEnter={e => { if (!sel) e.currentTarget.style.background = 'var(--sf-h)'; }}
                      onMouseLeave={e => { if (!sel) e.currentTarget.style.background = 'transparent'; }}
                    >
                      <div style={{
                        width: 20, height: 20, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center',
                        border: sel ? '2px solid #8B5CF6' : '2px solid var(--bd)',
                        background: sel ? '#8B5CF6' : 'transparent',
                        transition: 'all 0.1s', flexShrink: 0,
                      }}>
                        {sel && <Check size={12} color="#fff" />}
                      </div>
                      <Database size={14} color={sel ? '#8B5CF6' : 'var(--tx3)'} />
                      <span style={{ fontSize: 13, fontWeight: sel ? 600 : 400, color: sel ? 'var(--tx)' : 'var(--tx2)' }}>{db}</span>
                      {isSystem && (
          ...
          {step === 'schedule' && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Harmonogram backupu</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 20px' }}>Jak często agent ma tworzyć kopię zapasową?</p>
                ...
                {SCHEDULE_PRESETS.map(s => (
                  <button key={s.value} onClick={() => setSchedule(s.value)}
                    style={{
                      ...
                      border: schedule === s.value ? '2px solid var(--pri)' : '1px solid var(--bd)',
                      background: schedule === s.value ? 'rgba(99,102,241,0.06)' : 'transparent',
                      transition: 'all 0.1s',
                    }}
                    onMouseEnter={e => { if (schedule !== s.value) e.currentTarget.style.background = 'var(--sf-h)'; }}
                    onMouseLeave={e => { if (schedule !== s.value) e.currentTarget.style.background = 'transparent'; }}
                  >
                    <div style={{ fontSize: 13, fontWeight: 600, color: schedule === s.value ? 'var(--pri)' : 'var(--tx)' }}>
                      {s.label}
                    </div>
          ...
          {step === 'destinations' && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Gdzie zapisywać kopie?</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 20px' }}>Wybierz jedno lub więcej miejsc docelowych. Można łączyć np. folder lokalny + Google Drive.</p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{
                  ...
                  border: useInfradeskCloud ? '2px solid #6366F1' : '1px solid var(--bd)',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--tx3)' }}>Bezpieczne przechowywanie na serwerach InfraDesk — bez konfiguracji</div>
                    </div>
                ...
                {/* ── 1. Local / Network ── */}
                <div style={{ padding: 16, borderRadius: 14, border: '1px solid var(--bd)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: localPath ? 10 : 0 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)' }}>Folder lokalny lub sieciowy</div>
                      <div style={{ fontSize: 10, color: 'var(--tx3)' }}>Dysk lokalny, sieciowy (UNC), zamapowany dysk</div>
                    </div>
                  </div>
                  ...
                  <p style={{ fontSize: 10, color: 'var(--tx3)', marginTop: 4 }}>Pozostaw puste aby użyć domyślnego folderu agenta</p>
                </div>

                {/* ── 2. Google Drive ── */}
                <div style={{ padding: 16, borderRadius: 14, border: useGdrive ? '2px solid #22C55E' : '1px solid var(--bd)', background: useGdrive ? 'rgba(34,197,94,0.03)' : 'transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)' }}>Google Drive</div>
                      <div style={{ fontSize: 10, color: 'var(--tx3)' }}>Automatyczny upload kopii do chmury Google</div>
                    </div>
                    ...
                    <div>
                      <div style={{
                        ...
                        <button onClick={() => { setGdriveEmail(''); setGdriveRefreshToken(''); }}
                          style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--tx3)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
                          Rozłącz
                        </button>
                      </div>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx2)', marginBottom: 4, display: 'block' }}>
                        ...
                        <p style={{ fontSize: 10, color: 'var(--tx3)', marginTop: 4 }}>
                          ...
                          <a href="https://drive.google.com/drive/my-drive" target="_blank" rel="noopener noreferrer"
                            style={{ color: 'var(--pri)', textDecoration: 'underline', fontWeight: 600 }}>
                            Otwórz Google Drive ↗
                          </a>
                        </p>
                      </div>
                ...
                {/* ── 3. FTP / SFTP ── */}
                <div style={{ padding: 16, borderRadius: 14, border: useFtp ? '2px solid #8B5CF6' : '1px solid var(--bd)', background: useFtp ? 'rgba(139,92,246,0.03)' : 'transparent' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: useFtp ? 10 : 0 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)' }}>FTP / SFTP</div>
                      <div style={{ fontSize: 10, color: 'var(--tx3)' }}>Upload na zdalny serwer FTP lub SFTP</div>
                    </div>
                  ...
                  {useFtp && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx2)', marginBottom: 4, display: 'block' }}>Host</label>
                        <input style={inputStyle} value={ftpHost} onChange={e => setFtpHost(e.target.value)} placeholder="ftp.firma.pl" />
                      </div>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx2)', marginBottom: 4, display: 'block' }}>Port</label>
                        <input style={inputStyle} type="number" value={ftpPort} onChange={e => setFtpPort(e.target.value)} placeholder="21 (FTP) / 22 (SFTP)" />
                      </div>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx2)', marginBottom: 4, display: 'block' }}>Użytkownik</label>
                        <input style={inputStyle} value={ftpUser} onChange={e => setFtpUser(e.target.value)} placeholder="backup_user" />
                      </div>
                      <div>
                        <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx2)', marginBottom: 4, display: 'block' }}>Hasło</label>
                        <input style={inputStyle} type="password" value={ftpPass} onChange={e => setFtpPass(e.target.value)} placeholder="••••••••" />
                      </div>
                      <div style={{ gridColumn: '1/-1' }}>
                        <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx2)', marginBottom: 4, display: 'block' }}>Ścieżka zdalna</label>
          ...
          {step === 'notifications' && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Powiadomienia email</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 20px' }}>Kto ma otrzymać powiadomienie o statusie backupu? (opcjonalnie)</p>
              ...
              <div style={{ padding: 16, borderRadius: 14, border: '1px solid rgba(239,68,68,0.15)', background: 'rgba(239,68,68,0.03)', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)' }}>Przy błędzie backupu</div>
              ...
              {/* Success email */}
              <div style={{ padding: 16, borderRadius: 14, border: '1px solid var(--bd)', marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx)' }}>Przy udanym backupie</div>
                    <div style={{ fontSize: 10, color: 'var(--tx3)' }}>Opcjonalnie — potwierdzenie po każdej kopii</div>
                  </div>
                </div>
          ...
          {step === 'options' && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Opcje backupu</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 20px' }}>Nazwa, szyfrowanie i retencja</p>
              ...
              {/* Encryption */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16, padding: 16, borderRadius: 14, border: '1px solid var(--bd)' }}>
                ...
                {encrypt && (
                  <div>
                    <label style={{ ...labelStyle, display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ color: 'var(--tx3)', fontWeight: 400, textTransform: 'none' }}>— zachowaj w bezpiecznym miejscu!</span>
                    </label>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button onClick={copyKey} title="Kopiuj" style={{
                        ...
                        background: keyCopied ? 'rgba(34,197,94,0.1)' : 'var(--sf-h)',
                        color: keyCopied ? '#22C55E' : 'var(--tx2)', cursor: 'pointer',
                      }}>
                        {keyCopied ? <Check size={14} /> : <Copy size={14} />}
                      </button>
                      <button onClick={genKey} title="Generuj nowy" style={{
                        padding: '8px 12px', borderRadius: 10, border: '1px solid var(--bd)',
                        background: 'var(--sf-h)', color: 'var(--tx2)', cursor: 'pointer', fontSize: 11,
                      }}>
                        Nowy
                      </button>
                    </div>
          ...
          {step === 'summary' && (
            <div>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--tx)', margin: '0 0 4px' }}>Podsumowanie konfiguracji</h3>
              <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 20px' }}>Sprawdź i zatwierdź konfigurację backupu</p>

              <div style={{ borderRadius: 14, border: '1px solid var(--bd)', padding: '4px 18px', marginBottom: 20 }}>
          ...
          {step === 'done' && (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              ...
              <h3 style={{ fontSize: 20, fontWeight: 700, color: 'var(--tx)', margin: '0 0 8px' }}>
                Backup skonfigurowany!
              </h3>
              <p style={{ fontSize: 13, color: 'var(--tx2)', marginBottom: 8, maxWidth: 360, margin: '0 auto 24px' }}>
