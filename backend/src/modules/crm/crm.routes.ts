...
const router = Router();
router.use(authenticate, requireWorkspace, requireFeature('crm'));

router.get('/',                    withWorkspaceMembership, authorizeWorkspace('OWNER', 'ADMIN', 'TECHNICIAN'), getActivities);
router.get('/timeline',            withWorkspaceMembership, authorizeWorkspace('OWNER', 'ADMIN', 'TECHNICIAN'), getTimeline);
