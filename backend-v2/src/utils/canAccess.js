...
const ADMIN_DEFAULT_LEVEL = 'DELETE';
const OWNER_DEFAULT_LEVEL = 'DELETE';
const LEVEL_ORDER = { NONE: 0, VIEW: 1, EDIT: 2, DELETE: 3 };
function satisfies(have, need) {
    const map = { view: 'VIEW', edit: 'EDIT', delete: 'DELETE' };
    return LEVEL_ORDER[have] >= LEVEL_ORDER[map[need]];
}
export function effectiveLevel(ctx, moduleKey) {
    if (ctx.isSuperAdmin)
        return 'DELETE';
    ...
    if (ctx.role === 'OWNER')
        return OWNER_DEFAULT_LEVEL;
    if (ctx.role === 'ADMIN')
        return ADMIN_DEFAULT_LEVEL;
    // MEMBER
    return DEFAULT_MEMBER_LEVELS[moduleKey] ?? 'VIEW';
}
export function canAccess(ctx, moduleKey, action = 'view') {
    const level = effectiveLevel(ctx, moduleKey);
    return satisfies(level, action);
}
export function canAccessResource(ctx, moduleKey, action, resource) {
...
export function visibleModules(ctx) {
