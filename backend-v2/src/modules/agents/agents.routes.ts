...
async function authAgent(req: Request): Promise<{ registrationId: string; deviceId: string | null; workspaceId: string; status: string }> {
  ...
  const reg = await prismaBg.agentRegistration.findUnique({
...
router.post('/register', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const workspace = await prismaBg.workspace.findUnique({
    ...
    const existing = await prismaBg.agentRegistration.findFirst({
    ...
    const reg = await prismaBg.agentRegistration.create({
      data: {
...
adminRouter.get('/', requireAccess(MODULES.DEVICES, 'view'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // MSP cross-read: include agents from client workspaces where caller is provider with canViewDevices
    const clientWsRels = await prisma.workspaceRelation.findMany({
      where: {
    ...
    const accessibleWsIds = [req.workspaceId!, ...clientWsRels.map((r) => r.clientWorkspaceId)];
    const where: Record<string, unknown> = { workspaceId: { in: accessibleWsIds } };
    ...
    const agents = await prisma.agentRegistration.findMany({
      ...
      select: {
        ...
        createdAt: true, deviceId: true, workspaceId: true, workspace: { select: { id: true, name: true, type: true } },
      },
    });
    res.json({ agents });
  } catch (err) { next(err); }
...
async function findAgentInWorkspace(agentId: string, workspaceId: string) {
  // MSP cross-read: caller (workspaceId) may be provider for client workspaces
  // with active WorkspaceRelation + canViewDevices. We include those too so
  // push-command / run-speedtest / etc work for agents in client workspaces.
  const rels = await prisma.workspaceRelation.findMany({
    where: {
  ...
  const accessibleWsIds = [workspaceId, ...rels.map((r) => r.clientWorkspaceId)];
  return prisma.agentRegistration.findFirst({
    where: { id: agentId, workspaceId: { in: accessibleWsIds } },
    select: {
...
adminRouter.post('/:id/push-command', requireAccess(MODULES.DEVICES, 'edit'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const payload: Record<string, unknown> = { ...(body.payload ?? {}), type: body.type };
    switch (body.type) {
