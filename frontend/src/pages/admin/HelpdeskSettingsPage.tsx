...
import { Wrench, Save, Loader2, Check } from 'lucide-react';
...
export default function HelpdeskSettingsPage() {
  ...
  const { data: relations } = useQuery({
    ...
    queryFn: () => apiClient.get<{ asClient: ProviderRelation[] }>('/api/workspace-relations').then(r => r.data.asClient ?? []),
  });
  ...
  const mutation = useMutation({
    mutationFn: (data: Partial<HelpdeskSettings>) =>
      apiClient.put('/api/helpdesk-settings', data).then(r => r.data),
    onSuccess: () => {
  ...
  return (
    <div style={{ padding: '0 0 40px' }}>
      <PageHeader
        title="Ustawienia Help Desk"
        subtitle="Konfiguracja routingu zgłoszeń i integracji"
        actions={
          <button
      ...
      <div className="page-card" style={{ padding: 24, marginBottom: 20 }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--tm)', margin: '0 0 16px' }}>
          Zdecyduj, jak system przekierowuje nowe zgłoszenia
        </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {ROUTING_MODES.map(rm => {
            ...
            return (
              <button
                key={rm.id}
                onClick={() => setForm(f => ({ ...f, ticketRoutingMode: rm.id }))}
                style={{
      ...
      {form.ticketRoutingMode === 'send_to_default_provider' && (
        <div className="page-card" style={{ padding: 24, marginBottom: 20 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--t)', margin: '0 0 12px' }}>
            Domyślna firma IT
          </h3>
          {providers.length > 0 ? (
            <select
              className="input"
              value={form.defaultProviderWorkspaceId ?? ''}
              onChange={e => setForm(f => ({ ...f, defaultProviderWorkspaceId: e.target.value || null }))}
              style={{ minWidth: 300 }}
            >
              <option value="">— Wybierz firmę IT —</option>
              {providers.map(p => (
                <option key={p.providerWorkspace.id} value={p.providerWorkspace.id}>
                  {p.providerWorkspace.name}
                </option>
              ))}
            </select>
          ) : (
            <p style={{ fontSize: 13, color: 'var(--tm)' }}>
              Brak połączonych firm IT. Dodaj relację w sekcji Udostępnianie lub poczekaj na zaproszenie od firmy IT.
            </p>
          )}
        </div>
      )}
      ...
      <div className="page-card" style={{ padding: 24 }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--t)', margin: '0 0 16px' }}>
          Opcje dodatkowe
        </h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <ToggleRow
            label="Użytkownik może wybrać firmę IT"
            desc="Pozwól użytkownikom ręcznie wybrać, do której firmy IT trafi zgłoszenie"
            checked={form.allowUserProviderSelection}
            onChange={v => setForm(f => ({ ...f, allowUserProviderSelection: v }))}
          />
          <ToggleRow
            label="Asystent AI może tworzyć zgłoszenia"
            desc="AI automatycznie tworzy zgłoszenia na podstawie wykrytych problemów"
            checked={form.allowAssistantAutoCreate}
            onChange={v => setForm(f => ({ ...f, allowAssistantAutoCreate: v }))}
          />
          <ToggleRow
            label="Alerty agentów tworzą zgłoszenia"
            desc="Automatyczne tworzenie zgłoszeń gdy agent wykryje problem"
            checked={form.allowAlertAutoCreate}
            onChange={v => setForm(f => ({ ...f, allowAlertAutoCreate: v }))}
          />
        </div>
      </div>
    </div>
