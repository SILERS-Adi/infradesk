/**
 * IDS 1.0 — New Shipment Form (ModuleFormTemplate)
 * Connected to: POST /api/packaging/shipments
 */
...
export function ShipmentNewPage() {
  ...
  async function handleSubmit(status: 'PENDING' | 'PACKING') {
    if (!validate()) return;
    setSaving(true);
    try {
      const payload = {
  ...
  return (
    <>
      <PageHeader title="Nowa przesyłka" subtitle="Utwórz nową paczkę do wysyłki" back="/packaging/shipments" />
      <div style={{ padding: '0 24px 120px', maxWidth: 920, margin: '0 auto' }}>
                  ...
                  {items.map((item, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid var(--border)' }}>
                      <td style={{ padding: '8px 10px', minWidth: 200 }}>
                        <input placeholder="Nazwa produktu" value={item.name} onChange={e => updateItem(idx, 'name', e.target.value)}
        ...
        <div style={{ marginTop: 20 }}>
          <Card title="Uwagi">
            <Textarea label="Notatki" placeholder="np. Ostrożnie — szkło" value={notes} onChange={e => setNotes(e.target.value)} />
          </Card>
        </div>
      </div>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 20, background: 'var(--bg2)', borderTop: '1px solid var(--border)', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10 }}>
        <Button variant="ghost" onClick={() => navigate('/packaging/shipments')} disabled={saving}>Anuluj</Button>
        <Button variant="secondary" icon={<Save size={14} />} onClick={() => handleSubmit('PENDING')} loading={saving}>Zapisz</Button>
        <Button variant="primary" icon={<Send size={14} />} onClick={() => handleSubmit('PACKING')} loading={saving}>Rozpocznij pakowanie</Button>
      </div>
    </>
  );
}
