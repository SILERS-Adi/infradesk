    ...
    <div className="pakops">
      ...
      <div style={{ padding: '0 24px 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 24 }}>
          ...
          <KpiCard label="Przychód (okres)" value={`${fmtMoney(totalRevenue)} zł`} icon={<TrendingUp size={20} color="#fff" />} color="var(--success)" />
          <KpiCard label="Średnia wartość" value={`${fmtMoney(avgOrderValue)} zł`} icon={<BarChart3 size={20} color="#fff" />} color="#60A5FA" />
          <KpiCard label="Przychód dzisiaj" value={`${fmtMoney(s.revenueToday || 0)} zł`} icon={<Calendar size={20} color="#fff" />} color="var(--warning)" />
            ...
            {loadingChart ? <LoadingSpinner /> : chartData.length === 0 ? (
              ...
              <div>
                <div style={{ display: 'flex', gap: 16, marginBottom: 12, fontSize: 11, color: 'var(--tm)' }}>
                  ...
                  <span><span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: 2, background: 'var(--success)', marginRight: 4 }} />Przychód</span>
                </div>
                    ...
                    return (
                      <div key={i} style={{ flex: '1 1 0', minWidth: 0, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        <div title={`${d.date}: ${d.orders} zam. / ${fmtMoney(d.revenue)} zł`}
                          ...
                          <div style={{ width: '40%', maxWidth: 14, height: hRevenue, borderRadius: '3px 3px 0 0', background: 'var(--success)', opacity: 0.7 }} />
                        </div>
                        {chartData.length <= 14 && (
