...
- **2026-05-18 (#8 — Monitoring MSP cross-workspace FIX)** — Owner zapytał "dlaczego tylko 2 urządzenia na /monitoring" — okazało się że oba `/monitoring/overview` i `/monitoring/network` filtrowały tylko po `req.workspaceId`, ignorując 10 klientów MSP. Fix: helper `visibleWorkspaceIds()` agreguje caller + clientWorkspaceIds z aktywnych WorkspaceRelation (canViewDevices=true). Live-verified: **2 → 60 urządzeń** widocznych, pogrupowane po firmach (WISMONT 10, PKS Garwolin 16, Biuro Beata Polak 7, PakMar 7, METBUD 6, PC PLAST 4, BHP 3, CEDAR 2, DOMINEX 2, Silers 2, Dwór Osmolice 1), 280 alertów, 12 online / 48 offline. Commit `32a73a2`.
